...
final class ShipperController
{
    public static function ensureTables(PDO $pdo): void
    {
        ...
        self::ensureShipperColumn($pdo, 'description', 'TEXT NULL');
    ...
    public static function register(PDO $pdo, array $data, array $files): void
    {
        ...
        $stmt = $pdo->prepare(
            'INSERT INTO shippers (name, phone, password, avatar, description, is_active)
             VALUES (:name, :phone, :password, :avatar, :description, 1)'
        );
        $stmt->execute([
            'name' => $name,
            'phone' => $phone,
    ...
    public static function updateProfile(PDO $pdo, int $shipperId, array $data, array $files): array
    {
        ...
        $stmt = $pdo->prepare(
            "UPDATE shippers
             SET name = :name, phone = :phone, avatar = :avatar, description = :description {$passwordSql}
             WHERE id = :id"
        );
        $stmt->execute($params);

        $freshStmt = $pdo->prepare('SELECT id, name, phone, avatar, description, is_active, created_at FROM shippers WHERE id = :id LIMIT 1');
    ...
    public static function skip(PDO $pdo, int $orderId, int $shipperId, string $reason): void
    {
        ...
        $order = StaffOrder::find($pdo, $orderId);
    ...
    private static function ensureShipperColumn(PDO $pdo, string $column, string $definition): void
    {
        $stmt = $pdo->prepare(
            "SELECT COUNT(*)
             FROM INFORMATION_SCHEMA.COLUMNS
        ...
        if ((int)$stmt->fetchColumn() === 0) {
            $pdo->exec("ALTER TABLE shippers ADD COLUMN {$column} {$definition}");
        }
    }
