...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if ($action === 'delete_category') {
            $id = requirePositiveInt('id');
            $stmt = $pdo->prepare('SELECT COUNT(*) FROM foods WHERE category_id = :id');
            $stmt->execute(['id' => $id]);
            $foodCount = (int)$stmt->fetchColumn();
            ...
            if ($foodCount > 0) {
                ...
                $stmt->execute(['id' => $id]);
                $pdo->commit();
                setFlash('success', 'Danh mục đang có món nên đã ẩn danh mục và ẩn toàn bộ món thuộc danh mục này.');
            } else {
        ...
        if ($action === 'update_order_status') {
            ...
            $order = Order::find($pdo, $id);
            if (!$order || !canAdminTransitionOrderStatus($order['status'], $nextStatus)) {
        ...
        if ($action === 'cancel_order') {
            ...
            $stmt = $pdo->prepare("UPDATE orders SET status = 'Cancelled By Admin' WHERE id = :id");
            $stmt->execute(['id' => $id]);
            logActivity($pdo, 'order', 'Admin hủy đơn ' . $order['order_code'], ['id' => $id]);
        ...
        if ($action === 'save_shipper') {
            ...
            if ($id > 0) {
                ...
                $sql = 'UPDATE shippers SET name = :name, phone = :phone, avatar = :avatar, is_active = :is_active';
                if ($password !== '') {
                ...
                $stmt = $pdo->prepare('INSERT INTO shippers (name, phone, password, avatar, is_active) VALUES (:name, :phone, :password, :avatar, :is_active)');
                $stmt->execute([
                    'name' => $name,
                    'phone' => $phone,
    ...
    <div class="admin-main">
        <header class="admin-topbar">
            ...
            <div class="dropdown">
                ...
                <ul class="dropdown-menu dropdown-menu-end shadow-sm">
                    <li><span class="dropdown-header">Thông báo mới</span></li>
                    <li><a class="dropdown-item" href="<?= e(adminUrl('orders')) ?>">Đơn hàng mới đang chờ xử lý</a></li>
                    <li><a class="dropdown-item" href="<?= e(adminUrl('orders')) ?>">Có đơn đã chuẩn bị cần giao</a></li>
                    <li><a class="dropdown-item" href="<?= e(adminUrl('vouchers')) ?>">Kiểm tra voucher sắp hết hạn</a></li>
                </ul>
            </div>
        ...
        <main class="admin-content">
            ...
            <?php if ($page === 'dashboard'): ?>
                ...
                $statusLabels = ['Đã đặt', 'Đã hủy', 'Đã giao'];
                $statusValues = [
                    ...
                    <div class="col-lg-8"><div class="card"><div class="card-body table-responsive">
                        ...
                        <?php foreach ($categories as $category): ?>
                            ...
                            <?= csrfField() ?>
                                    ...
                                    <form class="d-inline" method="post" onsubmit="return confirm('Bạn chắc chắn muốn xóa danh mục này? Nếu danh mục có món, hệ thống sẽ ẩn danh mục và ẩn các món liên quan.');">
                            <?= csrfField() ?>
            ...
            <?php if ($page === 'foods'): ?>
                ...
                $foods = $pdo->query('SELECT f.*, c.name AS category_name FROM foods f JOIN categories c ON c.id = f.category_id ORDER BY f.is_deleted, f.created_at DESC')->fetchAll();
                ?>
                <div class="row g-4">
                    <div class="col-xl-4"><div class="card"><div class="card-body">
                        ...
                        <form method="post" enctype="multipart/form-data">
                            ...
                            <input class="form-control mb-2" name="price" type="number" min="0" step="1000" value="<?= e((string)($foodEdit['price'] ?? '')) ?>" required>
            ...
            <?php if ($page === 'orders'): ?>
                ...
                $viewOrder = $viewOrderId > 0 ? Order::find($pdo, $viewOrderId) : null;
                $orderItems = $viewOrder ? Order::items($pdo, $viewOrderId) : [];
                ...
                $orders = $searchQ !== '' ? Order::search($pdo, $searchQ) : Order::allWithShipper($pdo);
                ...
                <?php if ($viewOrder): ?>
                    <div class="card mb-4"><div class="card-body">
                        <div class="d-flex justify-content-between flex-wrap gap-2">
                            <div><h1 class="h5 page-title">Đơn hàng <?= e($viewOrder['order_code']) ?></h1><div class="text-muted"><?= e($viewOrder['customer_name']) ?> - <?= e($viewOrder['phone']) ?> - <?= e($viewOrder['address']) ?></div></div>
            ...
            <?php if ($page === 'shippers'): ?>
                ...
                <div class="row g-4">
                    ...
                    <div class="col-lg-8"><div class="card"><div class="card-body table-responsive">
                        <table class="table table-modern align-middle"><thead><tr><th>Tên shipper</th><th>Số điện thoại</th><th>Trạng thái</th><th></th></tr></thead><tbody>
