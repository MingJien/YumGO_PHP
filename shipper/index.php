...
$shipperStmt = $pdo->prepare('SELECT id, name, phone, avatar, is_active, created_at FROM shippers WHERE id = :id LIMIT 1');
...
$myOrders = Order::byShipper($pdo, $shipperId);
        ...
        <div>
            <section class="driver-section-card mb-4" id="orders">
                ...
                <?php foreach ($readyOrders as $index => $order): ?>
                    <?php $items = Order::items($pdo, (int)$order['id']); ?>
                            ...
                            <tr>
                                ...
                                <td class="text-end driver-row-actions">
                                    ...
                                    <?php if ($order['status'] === 'Delivering'): ?>
                                        ...
                                        <button class="btn btn-sm btn-success" type="button" data-bs-toggle="modal" data-bs-target="#driverCompleteModal" data-order-id="<?= (int)$order['id'] ?>" data-order-code="<?= e($order['order_code']) ?>" data-order-total="<?= e(shipperMoney($order['total'])) ?>">Hoàn thành</button>
                                    <?php endif; ?>
                                </td>
                            </tr>
            ...
            <section class="driver-section-card mb-4" id="income">
                <div class="driver-section-title"><h2>Thống kê số lượng đơn đã giao</h2><div class="dropdown"><button class="btn btn-sm btn-outline-primary dropdown-toggle" type="button" data-bs-toggle="dropdown"><?= e($incomeRange['label']) ?></button><ul class="dropdown-menu dropdown-menu-end"><?php foreach ($rangeOptions as $key => $option): ?><li><a class="dropdown-item <?= $selectedIncomeRange === $key ? 'active' : '' ?>" href="<?= e(shipperRangeUrl('income_range', $key)) ?>"><?= e($option['label']) ?></a></li><?php endforeach; ?></ul></div></div>
                <div class="delivery-count-summary mb-3"><span class="driver-stat-icon"><i class="bi bi-check2-circle"></i></span><div><div class="text-muted small">Tổng đơn đã giao trong <?= e($incomeRange['label']) ?></div><strong><?= (int)$deliveredOrdersInSelectedRange ?> đơn</strong></div></div>
                ...
                <div class="table-responsive">
                    <table class="table table-modern align-middle">
                        ...
                        <?php if (!$historyOrders): ?>
                            <tr><td colspan="6" class="text-muted">Chưa có đơn hoàn thành hoặc đã hủy trong khoảng này.</td></tr>
                        <?php endif; ?>
                        </tbody>
                    </table>
                </div>
        ...
        <form method="post" class="modal-content">
            ...
            <div class="modal-header">
                <div>
                    ...
                    <div class="text-muted small" id="driverCompleteOrderCode">Đơn sẽ được tính vào thu nhập sau khi hoàn thành</div>
                </div>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Đóng"></button>
            </div>
            <div class="modal-body">
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-light" data-bs-dismiss="modal">Hủy</button>
                <button type="submit" class="btn btn-success"><i class="bi bi-check2-circle"></i> Xác nhận đã giao</button>
            </div>
        </form>
    </div>
</div>
...
if (incomeCanvas) {
    new Chart(incomeCanvas, {
        ...
        data: {
            ...
            datasets: [
                { label: 'Số đơn đã giao', data: <?= json_encode($orderCounts, JSON_UNESCAPED_UNICODE) ?>, backgroundColor: 'rgba(255,107,0,.78)', borderRadius: 10, maxBarThickness: 42 }
            ]
        },
        options: {
            responsive: true,
...
if (driverCompleteModal) {
    driverCompleteModal.addEventListener('show.bs.modal', (event) => {
        ...
        driverCompleteModal.querySelector('#driverCompleteOrderCode').textContent = orderCode ? `Đơn ${orderCode}` : 'Đơn sẽ được tính vào thu nhập sau khi hoàn thành';
