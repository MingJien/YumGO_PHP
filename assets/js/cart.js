...
function showToast(message, type = 'success') {
    ...
    Toast.fire({
        ...
        title: message,
        customClass: {
            popup: 'rounded-md shadow-card border border-light'
        }
    });
}
...
function initAJAXCartUpdates() {
    document.addEventListener('submit', function (e) {
        ...
        if (form && form.action.includes('page=cart-update')) {
            ...
            const row = form.closest('tr') || form.closest('.py-3.border-bottom');
...
function initAJAXCartRemoves() {
    document.addEventListener('click', function (e) {
        ...
        if (removeLink && removeLink.href.includes('page=cart-remove')) {
            ...
            const row = removeLink.closest('tr') || removeLink.closest('.py-3.border-bottom');
...
function checkEmptyCart() {
    const rows = document.querySelectorAll('table tbody tr, .d-block.d-md-none .py-3.border-bottom');
