...
<head>
    ...
    <style>
        ...
        .invoice-notice { max-width: 800px; margin: 20px auto; padding: 12px 16px; border: 1px solid #ffc107; background: #fff8e1; color: #6b4e00; border-radius: 8px; }
        .print-actions { max-width: 800px; margin: 20px auto; text-align: right; }
        .print-actions button { border: 0; border-radius: 999px; padding: 10px 18px; background: #ff6600; color: #fff; font-weight: 700; cursor: pointer; }
        @media print {
            .invoice-notice, .print-actions { display: none; }
            .invoice-box { box-shadow: none; border: 0; }
        }
    </style>
</head>
<body>
    <?php if (!empty($invoiceNotice)): ?>
        <div class="invoice-notice"><?= htmlspecialchars($invoiceNotice) ?></div>
        <div class="print-actions"><button type="button" onclick="window.print()">In hoặc lưu PDF</button></div>
    <?php endif; ?>
        ...
        <table class="info-table">
            <tr>
                ...
                <td width="50%" style="text-align: right;">
                    ...
                    <p>
                        <?= function_exists('orderStatusLabel') ? htmlspecialchars(orderStatusLabel($order['status'])) : htmlspecialchars($order['status']) ?>
                    </p>
                </td>
            </tr>
        </table>
