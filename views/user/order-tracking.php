            ...
            <?php if ($order): ?>
                ...
                <div class="card shadow-sm mb-4 order-card d-none d-md-block" style="border-radius: 16px; border: 1px solid var(--yumgo-hairline); background: var(--yumgo-surface-card);">
                    <div class="card-body py-4">
                        ...
                        <?php
                            $isPickupOrder = ($order['delivery_type'] ?? 'delivery') === 'pickup';
                            $statusList = $isPickupOrder
                        ...
                        <div class="tracking-timeline order-timeline py-4 position-relative text-center">
                            <?php if ($isCancelled): ?>
                                ...
                                <?php if (!empty($order['cancel_reason'])): ?>
                                    <div class="alert alert-danger text-start mx-auto" style="max-width: 620px;">
                        ...
                        <?php if (!empty($order['issue_reason'])): ?>
                            <div class="alert alert-warning mt-3 mb-0 text-start">
                                <strong>Shipper báo sự cố:</strong> <?= htmlspecialchars($order['issue_reason']) ?>
                                <?php if (!empty($order['issue_note'])): ?>
                                    <div class="small mt-1"><?= htmlspecialchars($order['issue_note']) ?></div>
                                <?php endif; ?>
                                <?php if (!empty($order['issue_reported_at'])): ?>
                                    <div class="small text-muted mt-1"><?= htmlspecialchars(date('d/m/Y H:i', strtotime($order['issue_reported_at']))) ?></div>
                                <?php endif; ?>
                            </div>
                        <?php endif; ?>
                        ...
                        <?php
                            ...
                            $canConfirmReceived = ($order['delivery_type'] ?? 'delivery') === 'pickup' && $currentStatus === 'Ready';
                        ?>
                        <?php if ($canCancel || $canEdit || $canConfirmReceived): ?>
                            <div class="mt-4 pt-3 border-top d-flex justify-content-end gap-2 order-action-group">
                                <?php if ($canConfirmReceived): ?>
                                    <form action="index.php?page=confirm-received" method="POST" class="m-0">
                                        <?= csrfField() ?>
                                        <input type="hidden" name="code" value="<?= htmlspecialchars($code) ?>">
                                        <button type="submit" class="btn btn-success">
                                            <i class="bi bi-check2-circle"></i> Xác nhận đã nhận
                                        </button>
                                    </form>
                                <?php endif; ?>
                                <?php if ($canEdit): ?>
                        ...
                        <div class="card-body p-3">
                            ...
                            <?php if ($isCancelled): ?>
                                <div class="alert alert-danger border-0 mb-4 text-center py-3 rounded-3" style="background: rgba(220, 53, 69, 0.08); color: #dc3545;">
                                    ...
                                    <?php if (!empty($order['cancel_reason'])): ?>
                                        <p class="mb-0 mt-2 small"><strong>Lý do:</strong> <?= htmlspecialchars($order['cancel_reason']) ?></p>
                                    <?php endif; ?>
                                </div>
                            <?php else: ?>
                            ...
                            <div class="mobile-actions d-grid gap-2">
                                <?php if ($canConfirmReceived): ?>
                                    <form action="index.php?page=confirm-received" method="POST" class="w-100 m-0">
                                        <?= csrfField() ?>
                                        <input type="hidden" name="code" value="<?= htmlspecialchars($code) ?>">
                                        <button type="submit" class="btn btn-success py-2 w-100" style="border-radius: 12px; font-weight: 600; font-size: 0.95rem;">
                                            <i class="bi bi-check2-circle"></i> Xác nhận đã nhận
                                        </button>
                                    </form>
                                <?php endif; ?>
